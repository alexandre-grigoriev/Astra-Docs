...
Andor camera is connected to Astra controller using USB interface and proprietary protocol.

![Andor detector connections](./img/AndorHW.svg)

Andor camera is equiped with TTL output which allows to control shutter.
_Note_: The instrument may be is equiped with several shutters (laser shutter, spectrometer shutter, etc.). Please check electrical diagrams.
